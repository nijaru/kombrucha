...
use kombrucha::{PackageManager, cellar, symlink};
...
async fn main() -> anyhow::Result<()> {
    ...
    assert!(installed.len() > 0, "Should have at least one package installed");
    ...
    println!("search('ripgrep'): {} results in {:.2}ms", 
    ...
    println!("Found {} outdated packages in {:.2}ms", 
        outdated.len(), 
    ...
    println!("  Space that would be freed: {:.2} MB", cleanup_result.space_freed_mb);
    println!("  Errors encountered: {}", cleanup_result.errors.len());
    println!("  Time to scan: {:.2}ms", cleanup_time.as_secs_f64() * 1000.0);
    ...
    println!("  ✓ cleanup() - Would free {:.2} MB", cleanup_result.space_freed_mb);
    println!("  ✓ uses() - Reverse dependency lookup");
    println!("  ✓ Type safety - All result types verified");
    
    println!("\nNext steps:");
    println!("  1. Run: cargo test --lib");
    println!("  2. For destructive tests (install/upgrade/uninstall), test manually:");
    println!("     cargo run --release -- install ripgrep");
    println!("     cargo run --release -- upgrade ripgrep");
    println!("     cargo run --release -- uninstall ripgrep");
